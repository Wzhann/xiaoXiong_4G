...
static void LogXiaoXiongMicLevel(const std::vector<int16_t>& data) {
    ...
    for (int16_t sample : data) {
        int32_t value = std::abs(static_cast<int32_t>(sample));
        if (value > peak) {
            peak = value;
        }
        sum += value;
        ...
        if (has_prev && ((prev_sample < 0 && sample >= 0) || (prev_sample >= 0 && sample < 0))) {
            zero_crossings++;
        }
        prev_sample = sample;
        has_prev = true;
    ...
    int32_t avg = data.empty() ? 0 : static_cast<int32_t>(sum / data.size());
    int32_t mean = data.empty() ? 0 : static_cast<int32_t>(signed_sum / data.size());
    ESP_LOGI(TAG, "mic level: samples=%u peak=%ld avg=%ld min=%d max=%d mean=%ld zcr=%lu",
             static_cast<unsigned>(data.size()), static_cast<long>(peak), static_cast<long>(avg),
             min_sample, max_sample, static_cast<long>(mean),
             static_cast<unsigned long>(zero_crossings));
}
#endif
