...
static esp_err_t panel_gc9d01n_init(esp_lcd_panel_t *panel){
    ...
    bool use_default_init_cmds = false;
    ...
    for (int i = 0; i < init_cmds_size; i++){
        ...
        if (is_cmd_overwritten){
            if (use_default_init_cmds) {
                ESP_LOGD(TAG, "The %02Xh command is set by the default initialization sequence", init_cmds[i].cmd);
            } else {
